...
# top left number = number of actual observations
# top right number = number of expected observations if truely random spatial distribution
# bottom number = error term (?)
```
...
# scary thing about denstiy is that its totally dependent on the bandwidth you choose (sigma), 
grouse_density <- density(grouse_full, sigma = 0.05)
plot(grouse_density)

# if you have no idea what to set bandwidth at, look at data and try some stuff out to see how closely you can get to match based on original data, or use existing literature values, 
